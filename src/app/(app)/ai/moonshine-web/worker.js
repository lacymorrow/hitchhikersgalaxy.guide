...
// Remove top-level await and wrap in initialization function
let isInitialized = false;
let silero_vad, transcriber, device;
...
async function initialize() {
	...
	try {
		...
		[silero_vad, transcriber] = await Promise.all([
			AutoModel.from_pretrained("onnx-community/silero-vad", {
				config: { model_type: "custom" },
				dtype: "fp32",
			}),
			pipeline("automatic-speech-recognition", "onnx-community/moonshine-base-ONNX", {
				device,
...
self.onmessage = async (event) => {
	await initialize(); // Ensure initialization before processing messages
