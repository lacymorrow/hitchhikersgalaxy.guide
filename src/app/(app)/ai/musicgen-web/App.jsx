...
// Configure WASM backend with error handling
try {
	env.backends.onnx.wasm.proxy = true;
	env.useBrowserCache = false; // Disable browser cache to prevent stale WASM modules
} catch (error) {
...
const App = () => {
	...
	const generateMusic = async () => {
		try {
			...
			if (!tokenizer || !model) {
				throw new Error(
					"Model or tokenizer not loaded. Please try refreshing the page.",
				);
			}
			...
			setError(err.message);
			setStatusText("Failed to generate music. Please try again.");
		}
	};
